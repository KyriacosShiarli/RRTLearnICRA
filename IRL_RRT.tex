...
Figure \ref{fig:res_kino1} shows that RLT$^*$ is an order of magnitude faster than RLT-NC. \sw{But it also shows lower cost difference.} The kinematic constraints contribute to this speedup since they make the \texttt{Steer} and \texttt{Safe} procedures, which are cached, more expensive. Figure \ref{fig:res_kino2} shows that RLT$^*$ produces in more stable learning and lower cost differences. \sw{What does the learning curve add exactly?}
